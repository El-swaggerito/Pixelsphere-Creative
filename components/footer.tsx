...
export default function Footer() {
  ...
  const [honeypot, setHoneypot] = useState(""); // simple anti-bot field
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);
  const [error, setError] = useState("");

  // Read once (prevents effect churn)
  const {
    serviceId,
    templateId,
    publicKey,
    ...
    () => ({
      serviceId: process.env.NEXT_PUBLIC_EMAILJS_NEWSLETTER_SERVICE_ID,
      templateId: process.env.NEXT_PUBLIC_EMAILJS_NEWSLETTER_TEMPLATE_ID,
      publicKey: process.env.NEXT_PUBLIC_EMAILJS_NEWSLETTER_PUBLIC_KEY,
    }),
    []
  );

  const validateEmail = useCallback((value: string) => {
    // RFC 5322-lite; your original pattern is fine, this is just slightly stricter
    const emailRegex =
  ...
  const handleNewsletterSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Basic bot check: real users won't see/fill this
    if (honeypot) return;
    ...
    try {
      // Lazy-load EmailJS only at submit time (smaller initial bundle)
      const emailjs = (await import("@emailjs/browser")).default;

      const now = new Date();
      const newsletterParams = {
        subscriber_email: trimmed,
        subscription_date: now.toISOString(), // unambiguous
        year: now.getFullYear(),
        to_email: "newsletter@pixelsphere.com", // keep as-is or move to env if desired
      };
      ...
      setIsSuccess(true);
      // Soft reset state after a short delay
      const t = setTimeout(() => {
        setIsSuccess(false);
        setEmail("");
        setError("");
      ...
      console.error("Failed to send newsletter subscription:", err);
      setError("Sorry, there was an error. Please try again.");
    } finally {
      setIsSubmitting(false);
    }
  }, [email, honeypot, publicKey, serviceId, templateId, validateEmail]);
  ...
  return (
    <footer className="bg-blue-700 text-white py-12 sm:py-16 overflow-hidden relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 lg:gap-12">
            ...
            <div className="mb-6 sm:mb-8">
              ...
              <div className="flex space-x-3 sm:space-x-4">
                {socialLinks.map(({ href, label, Icon }) => (
                  <motion.div key={label} whileHover={{ scale: 1.2, rotate: 360 }} transition={{ duration: 0.3 }}>
                    <Link
                      href={href}
                      rel="noopener noreferrer nofollow"
                      target="_blank"
                      className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
                      aria-label={label}
                    >
                      <Icon className="text-white w-4 h-4 sm:w-5 sm:h-5" aria-hidden="true" />
                    </Link>
                  </motion.div>
                ))}
              </div>
          ...
          <div className="lg:col-span-1">
            ...
            <ul className="space-y-3 sm:space-y-4 text-blue-200 text-sm sm:text-base">
              {[
                { href: "/services/custom-web-solutions", text: "Web Design & Development" },
                { href: "/services/ux-design-strategy", text: "UI UX Design" },
                { href: "/services/branding-identity", text: "Brand Identity" },
                { href: "/services/graphic-design", text: "Graphics Design" },
                { href: "/services/digital-growth", text: "Digital Growth" },
              ].map((item) => (
                <li key={item.href}>
          ...
          <div className="lg:col-span-1">
            <div className="bg-orange-500 rounded-lg p-6 relative overflow-hidden">
              ...
              <div className="absolute top-0 right-0 w-20 h-20 bg-orange-400/10 rounded-full -translate-y-10 translate-x-10" />
              <div className="absolute bottom-0 left-0 w-16 h-16 bg-orange-600/10 rounded-full translate-y-8 -translate-x-8" />
              ...
              <div className="relative z-10">
                ...
                >
                  ...
                  <div>
                    ...
                    />
                    {error && (
                      <p id="newsletter-error" className="text-red-100 text-xs mt-1 bg-red-500/20 px-2 py-1 rounded">
                        {error}
                      </p>
                    )}
                  </div>
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      <div className="flex items-center justify-center">
                        <Mail className="w-3 h-3 mr-2" aria-hidden="true" />
